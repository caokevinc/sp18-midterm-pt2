...
class App extends Component {
  ...
  watchLogs() {
    ...
    transactionCreatedEvent.watch(function(error, result) {
      if (error) {
        ...
        /*
        that.state.events[that.state.numEvents] = result.event;
        that.state.numEvents++;
        */
        that.setState({ events: [that.state.events + result.event], numEvents: that.state.events + 1 });
        console.log(result.event);
      }
  ...
  sendEther() {
    console.log("doggo");
    this.state.web3.eth.sendTransaction({from:'0xa0b202191aaaf3b9c032abb82fb0cf260bc5b4baead26beb3b14c6c06ff5259e', to: this.state.multiSigContract.address, value: 500});
